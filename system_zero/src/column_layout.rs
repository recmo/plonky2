...
pub(crate) const fn col_permutation_state(round: usize, i: usize) -> usize {
    debug_assert!(round <= poseidon::N_ROUNDS);
    debug_assert!(i < SPONGE_WIDTH);
    START_PERMUTATION_UNIT + SPONGE_WIDTH * round + i
}

pub(crate) const fn col_permutation_input(i: usize) -> usize {
    col_permutation_state(0, i)
}

pub(crate) const fn col_permutation_output(i: usize) -> usize {
    col_permutation_state(poseidon::N_ROUNDS, i)
}
